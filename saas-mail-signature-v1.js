...
setTimeout(boot,250);window.addEventListener('load',()=>setTimeout(boot,100));window.addEventListener('lm:client-data-ready',schedule);window.addEventListener('lm:client-switched',schedule);document.addEventListener('click',e=>{if(e.target.closest?.('.nav button[data-view="leadmanager"]'))schedule()},true);document.addEventListener('lm:mail-connected',e=>setTimeout(()=>{if(!signaturePresent())discover(e?.detail?.account||'');else{render();msg('✓ Den gemte signatur er fundet og bruges på mails fra Lead Manager.')}},300));
