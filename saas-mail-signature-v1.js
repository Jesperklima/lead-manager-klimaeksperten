...
setTimeout(boot,250);window.addEventListener('load',()=>setTimeout(boot,100));document.addEventListener('lm:mail-connected',e=>setTimeout(()=>{if(!signaturePresent())discover(e?.detail?.account||'');else{render();msg('✓ Den gemte signatur er fundet og bruges på mails fra Lead Manager.')}},300));let n=0,t=setInterval(()=>{n++;boot();if($('#lmMailSignatureWrap')||n>50)clearInterval(t)},300);
