...
function legacyTrusted(){
 const s=settings();
 return !!html()&&(
  !!s.mail_signature_verified_at||
  !!s.mail_signature_source||
  !!s.mail_signature_account||
  !!s.mail_signature
 );
}
function verified(){return legacyTrusted()}
...
function install(){if(window.__lmSignatureV2)return;window.__lmSignatureV2=true;const original=window.fetch.bind(window);window.fetch=async function(input,init){const url=typeof input==='string'?input:(input?.url||'');let body=null;if(init?.body&&typeof init.body==='string')try{body=JSON.parse(init.body)}catch{};const providerCall=url.includes('/functions/v1/mail-provider-auth');const sending=/mail|gmail|microsoft/i.test(url)&&/(send|send_mail|send-email|mail_send)/i.test(url+' '+String(body?.action||''));if(sending&&!verified()){const account=String(body?.from||body?.account||settings().mail||'').trim();const ok=await ensureVerified(account);if(!ok)throw new Error('Mail-signaturen mangler. Gå til Mail-indstillinger og vælg “Find signatur igen”, eller kontrollér at mailkontoen er forbundet.')}if(sending&&body&&html()){body.signature_html=html();body.signature=html();body.signature_verified=true;init={...init,body:JSON.stringify(body)}}const r=await original(input,init);if(providerCall&&body?.action==='save'&&r.ok)setTimeout(()=>discover(body.account||''),500);return r}}
...
window.LMMailSignature={version:VERSION,discover,verified,ensureVerified,legacyTrusted};
})();
