...
function activeView(){
  return document.querySelector('.nav button[data-view].active')?.dataset?.view||'dashboard';
}
function safeCall(name){
  try{if(typeof window[name]==='function')window[name]()}catch(e){console.warn('perf render '+name,e)}
}
...
function refreshOpenSurface(changedKeys){
  ...
  try{
    if(document.getElementById('drawer')?.classList.contains('open')&&typeof currentLead!=='undefined'&&currentLead?.id&&
      ['companies','contacts','leads','activities','mail','intelligence','tasks'].some(k=>changed.has(k))){
      const id=currentLead.id;
      currentLead=(state.leads||[]).find(x=>x.id===id)||currentLead;
      if(typeof openLead==='function')openLead(id);
    }
  ...
  try{
    if(document.getElementById('offerModal')?.classList.contains('open')&&typeof currentOffer!=='undefined'&&currentOffer?.id&&changed.has('offers')){
      ...
      currentOffer=(state.offers||[]).find(x=>x.id===id)||currentOffer;
  ...
  const smartLoad=async function(options={}){
    ...
    refreshInFlight=(async()=>{
      if(forceFull){
        ...
        updateSyncLabel(13,performance.now()-started);
        return out;
      }
      const cid=state?.client?.id;
      if(!cid)return;
      let keys=[...new Set([...dirtyTables].map(t=>tableToKey[t]).filter(Boolean))];
      dirtyTables.clear();
      ...
      for(const [key,r] of results){
        if(r?.error){console.error('perf refresh '+key,r.error);if(typeof toast==='function')toast('Fejl ved '+key);continue}
        state[key]=r?.data||[];
      }
      lastRefreshAt=Date.now();
      renderView();
      refreshOpenSurface(keys);
      if(keys.includes('offers'))safeCall('renderOfferPipeline');
    ...
    try{return await refreshInFlight}
      ...
      if(pendingRefresh){
        const full=pendingFull;pendingRefresh=false;pendingFull=false;
        if(dirtyTables.size||full)setTimeout(()=>smartLoad({full}),0);
      }
    }
  };
  smartLoad.__lmPerfWrapped=true;
  smartLoad.full=()=>smartLoad({full:true});
...
function installNavigationRendering(){
  document.addEventListener('click',e=>{
    ...
    if(!b)return;
    setTimeout(()=>renderView(b.dataset.view),0);
  },true);
  const refresh=document.getElementById('refreshBtn');
  if(refresh)refresh.onclick=async()=>{await window.loadAll({full:true});if(typeof toast==='function')toast('CRM opdateret')};
}
...
function boot(){
  ...
  window.__LM_PERF={
    refreshFull:()=>window.loadAll({full:true}),
    refreshKeys:(...keys)=>window.loadAll({keys}),
