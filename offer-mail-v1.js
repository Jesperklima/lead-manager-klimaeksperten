(()=>{
  ...
  async function enrichFromMinuba(o){
    ...
    try{
      ...
      if(!data?.found){if(byId('offerMailContactSource'))byId('offerMailContactSource').textContent=o.contact_person?`Kontaktperson: ${o.contact_person}`:`Ingen kontaktperson fundet i Minuba på tilbud ${ref}.`;return}
      const person=String(data.contact_person||'').trim();if(person)o.contact_person=person;
      ...
      const incomingDetails=String(data.contact_details||'').trim(),cleanDetails=stripKnownBounced(incomingDetails,o);o.contact_details=cleanDetails;
      const patch={contact_person:o.contact_person||null,contact_details:cleanDetails||null,minuba_raw:o.minuba_raw,minuba_record_type:o.minuba_record_type,minuba_order_number:o.minuba_order_number,minuba_status:o.minuba_status,minuba_last_checked_at:o.minuba_last_checked_at,updated_at:new Date().toISOString()};
    ...
    }catch(error){console.warn('Minuba kontaktopslag fejlede',error);if(byId('offerMailContactSource'))byId('offerMailContactSource').textContent=o.contact_person?`Kontaktperson: ${o.contact_person}`:'Minuba kunne ikke hente kontaktpersonen lige nu.'}
  }
  ...
  async function sendMail(){
    ...
    try{
      ...
      byId('offerMailModal').classList.remove('open');if(typeof loadAll==='function')await loadAll();if(typeof openOffer==='function')openOffer(o.id);if(typeof toast==='function')toast(`Mail sendt til ${to}`);
