-- Lead Manager SaaS: Minuba is included in Pro and Business, but not Start.
-- Keeps production data and future crm_apply_plan calls aligned with SAAS_PACKAGES.md.
...
set allow_minuba = case when plan_code in ('pro','business','internal') then true else false end,
    updated_at = now()
where allow_minuba is distinct from (plan_code in ('pro','business','internal'));
