...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==='start'){
      ...
      const account=String(body.account||client.settings?.microsoft_mail||'').trim().toLowerCase();
      if(!emailOk(account))return json({error:'Indtast den Microsoft 365 / Outlook-mailkonto, der skal forbindes.',code:'MICROSOFT_ACCOUNT_INVALID'},400);
      const settings=client.settings||{};
      if(String(settings.microsoft_mail||'').trim().toLowerCase()!==account){
        const {error:updateError}=await admin.from('crm_clients').update({settings:{...settings,microsoft_mail:account}}).eq('id',clientId);if(updateError)throw updateError;
      }
      const returnUrl=safeReturnUrl(body.return_url,appUrl);
      ...
      const {error:stateError}=await admin.from('crm_microsoft_oauth_states').insert({state_token:stateToken,client_id:clientId,user_email:user.email,return_url:returnUrl,requested_scope:scope,code_verifier:codeVerifier});if(stateError)throw stateError;
      ...
      return json({authorize_url:'https://login.microsoftonline.com/common/oauth2/v2.0/authorize?'+params.toString(),account,scope,redirect_uri:REDIRECT_URI,return_url:returnUrl,client_name:client.name});
    }
