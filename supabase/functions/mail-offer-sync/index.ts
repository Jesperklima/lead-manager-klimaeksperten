...
async function runClient(admin:any,client:any,dryRun:boolean,backfillDays:number){
  ...
  if(!dryRun){
    ...
    await admin.from('crm_usage_events').insert({client_id:clientId,event_type:'mail_offer_sync',quantity:1,metadata:{fetched:allMessages.length,candidates:proposals.length,processed,ignored,approvals,minuba_created:minubaCreated,mode:'stored_first_closed_guard_v10'}});
  }
