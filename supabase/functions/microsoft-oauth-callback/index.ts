...
Deno.serve(async(req:Request)=>{
  try{
    const u=new URL(req.url),code=u.searchParams.get('code')||'',state=u.searchParams.get('state')||'',oauthError=u.searchParams.get('error')||'',oauthDesc=u.searchParams.get('error_description')||'';
    if(oauthError)return back('error','Microsoft returnerede: '+(oauthDesc||oauthError));
    if(!code||!state)return back('error','OAuth-svaret mangler kode eller state.');
    ...
    const row=states?.[0],returnUrl=safeReturn(row?.return_url);
    ...
    const expected=String(client?.settings?.mail||'').trim().toLowerCase();
    if(!expected||![graphMail,upn].filter(Boolean).includes(expected))return back('error',`Den valgte Microsoft-konto (${graphMail||upn||'ukendt'}) matcher ikke kundens mail (${expected||'mangler'}).`,returnUrl);
  ...
  }catch(err){console.error(err);return back('error',err instanceof Error?err.message:'Ukendt fejl')}
});
