...
async function markError(sb:any,job:any,error:any){
  const message=clean(error instanceof Error?error.message:String(error),3000);
  const done=new Date().toISOString();
...
async function mailSync(sb:any,url:string,clientId:string){
  ...
  if(!res.ok)throw new Error(clean(data?.error||text||('Mail sync HTTP '+res.status),2000));
  return data;
}
...
Brug den officielle kilde først. Afgør om muligheden stadig er aktiv, eller om projektet/udbuddet er afsluttet, tildelt, udført eller fristen er udløbet. Gæt ikke. Hvis kilden er uklar eller ikke kan verificeres, sæt confidence=low og active=false, closed=false.`;
  ...
  const patch:any={
    ...
    source_scope_verified:result.confidence!=='low'&&(result.active||result.closed),
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    return json({error:clean(error instanceof Error?error.message:String(error),3000)},500);
  }
});
